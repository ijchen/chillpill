...
pub struct PanicData {
    ...
    /// The source code location of the panic, or [`None`] if no source location
    /// was available.
    ///
    /// The current implementation of [`std::panic::PanicHookInfo::location`]
    /// always returns [`Some`], and as a result the current implementation of
    /// [`chillpill::catch`](crate::catch) always returns [`Some`]. This may
    /// change in a future Rust version, or even in a minor or patch update to
    /// `chillpill`.
...
impl PanicData {
    ...
    /// # Errors
    ///
    /// Returns `self` back if the panic payload was neither a `&str` nor a
    /// `String`.
    pub fn payload_into_string(self) -> Result<Cow<'static, str>, Self> {
        let Self { payload, location } = self;
...
mod tests {
    ...
    /// Produces something with a [`std::fmt::Debug`] impl matching what
    /// [`PanicData`] would be if it had a `#[derive(Debug)]` that formats the
    /// payload as `expected_payload`
    fn panic_data_debug_match(
        expected_payload: impl std::fmt::Debug + 'static,
        location: Option<PanicLocation>,
    ) -> impl std::fmt::Debug {
        /// A struct with an identical structure to [`super::PanicData`], used to
        /// make sure our manual [`std::fmt::Debug`] impl matches what
        /// `#[derive(...)]` would generate (besides our custom payload formatting)
        #[derive(Debug)]
        struct PanicData {
    ...
    /// Helper function to assert that two `impl std::fmt::Debug`s format
    /// identically across a range of format arguments
    ...
    /// This test ensures that [`PanicData`]'s manual [`std::fmt::Debug`] impl
    /// behaves identical to a `#[derive(..)]`'d impl, except that it formats
    /// [`&str`](str) payloads as a string.
    #[test]
    fn debug_formats_str_payload_correctly() {
        ...
        let data = make_panic_data("&str payload", location.clone());

        assert_debugs_match(data, panic_data_debug_match("&str payload", location));
    }

    /// This test ensures that [`PanicData`]'s manual [`std::fmt::Debug`] impl
    /// behaves identical to a `#[derive(..)]`'d impl, except that it formats
    /// [`String`] payloads as a string.
    #[test]
    fn debug_formats_string_payload_correctly() {
        ...
        let data = make_panic_data(String::from("String payload"), location.clone());

        assert_debugs_match(data, panic_data_debug_match("String payload", location));
    }

    /// This test ensures that [`PanicData`]'s manual [`std::fmt::Debug`] impl
    /// behaves identical to a `#[derive(..)]`'d impl, including formatting the
    /// payload as a Box<dyn Any> when it is neither a [`&str`](str) nor a
    /// [`String`].
    #[test]
    fn debug_formats_non_string_payload_correctly() {
        ...
        let data = make_panic_data((), location.clone());

        assert_debugs_match(
            data,
            panic_data_debug_match(Box::new(()) as Box<dyn Any>, location),
        );
    }

    /// This test ensures [`PanicData::payload_as_string`] correctly extracts a
    /// [`&str`](str) payload.
    #[test]
    fn payload_as_string_str() {
        let panic_data = make_panic_data("static str", None);
    ...
    fn payload_as_string_string() {
        let panic_data = make_panic_data(String::from("owned string"), None);

        assert_eq!(panic_data.payload_as_string(), Some("owned string"));
    }

    /// This test ensures [`PanicData::payload_as_string`] correctly returns
    /// [`None`] for a payload that is neither a [`&str`](str) nor a [`String`].
    #[test]
    fn payload_as_string_non_string() {
        let panic_data = make_panic_data(42u8, None);

        assert_eq!(panic_data.payload_as_string(), None);
    }

    /// This test ensures [`PanicData::payload_into_string`] correctly extracts
    /// a [`&str`](str) payload.
    #[test]
    ...
    /// This test ensures [`PanicData::payload_into_string`] correctly returns
    /// [`None`] for a payload that is neither a [`&str`](str) nor a [`String`].
    #[test]
    fn payload_into_string_non_string() {
        let panic_data = make_panic_data(1234u32, None);
